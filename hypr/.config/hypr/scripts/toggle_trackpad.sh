...
export HYPRLAND_DEVICE="apple-inc.-apple-internal-keyboard-/-trackpad-1"
...
if [ "$TOUCHPAD_ENABLED" != "false" ]; then
  ...
  hyprctl --batch -r -- keyword "cursor:inactive_timeout" 0.1
  ...
  hyprctl --batch -r -- keyword "cursor:inactive_timeout" -1
...
# notify-send -t 1000 -u normal "$NOTIFMSG"
